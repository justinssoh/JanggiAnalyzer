...
class JanggiBoardModel:
    def __init__(self):
        ...
        10행 9열의 2차원 리스트
        """
        # grid[row][col] 형태로 기물 문자를 저장
        # '.' 은 빈 칸을 의미
        self.grid = [['.' for _ in range(config.COLS)] for _ in range(config.ROWS)]
        
        # 기물 점수 합계나 잡힌 기물 목록 등을 추가로 관리 가능
        self.captured_pieces = {"w": [], "b": []}
        
        # 최초 생성 시 초기 배치(Reset)를 실행
        self.reset()

    def reset(self):
        """
        장기판 배열을 빈 칸으로만 초기화.
        기물 배치는 호출한 쪽에서 _parse_fen()으로 별도 수행.
        """
        for r in range(config.ROWS):
            for c in range(config.COLS):
                self.grid[r][c] = '.'
        self.captured_pieces = {"w": [], "b": []}
        print("BoardModel: 배열이 초기화되었습니다.")

    def _parse_fen(self, fen):
        """
        FEN(Forsyth-Edwards Notation) 문자열을 읽어 grid 배열에 채움
    ...
    def move_piece(self, r1, c1, r2, c2):
        """
        기물을 (r1, c1)에서 (r2, c2)로 이동
        이동 경로에 기물이 있었다면 잡힌 기물로 처리
        """
    ...
    def move_piece_uci(self, uci_move):
        """
        UCI 형식의 이동(e10e9)을 받아 기물을 이동시킴
    ...
    def generate_fen(self, turn='w'):
        """
        현재 grid 상태를 다시 FEN 문자열로 변환
        (엔진에게 현재 상황을 전달할 때 사용됨)
